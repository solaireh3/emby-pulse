    ...
    try:
        ...
        if c.fetchone()[0] == 0:
            ...
            default_store = [
                ...
                {"id": "renew_90", "type": "renew", "name": "账号续期 90 天", "cost": 1350, "val": 90, "icon": "fa-battery-three-quarters", "color": "text-teal-500", "desc": "九折优惠，安心看一季"},
                {"id": "renew_365", "type": "renew", "name": "账号续期 365 天", "cost": 5000, "val": 365, "icon": "fa-battery-full", "color": "text-blue-500", "desc": "年度尊享，极具性价比"}
            ]
            defaults = [
...
def batch_update_points(data: BatchPointsModel, request: Request):
    ...
    try:
        ...
        for uid in data.user_ids:
            ...
            c.execute("INSERT INTO point_logs (user_id, username, action, amount, balance) VALUES (?, ?, ?, ?, ?)", (uid, name_map.get(uid, "未知用户"), f"管理员批量操作: {data.reason}", data.amount, new_pts))
            count += 1
        conn.commit(); conn.close()
...
def user_redeem(data: RedeemModel, request: Request):
    ...
    try:
        ...
        if not target_item:
            conn.close(); return {"status": "error", "message": "商品不存在或已下架"}
        ...
        if current_points < cost:
            conn.close(); return {"status": "error", "message": f"余额不足！需要 {cost} 积分。"}
        ...
        if not current_exp or current_exp == "" or "2099" in current_exp:
            conn.close(); return {"status": "error", "message": "您的账号当前为【永久有效】，无需兑换续期服务！"}
        ...
        if target_item.get("type") == "renew":
            ...
            try: # 尝试解封
                requests.post(f"{cfg.get('emby_host')}/emby/Users/{user['Id']}/Policy?api_key={cfg.get('emby_api_key')}", json={"IsDisabled": False}, timeout=3)
            except: pass
        else:
